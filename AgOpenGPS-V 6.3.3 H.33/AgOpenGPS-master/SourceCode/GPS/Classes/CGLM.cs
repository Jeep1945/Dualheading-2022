...
namespace AgOpenGPS
{
    ...
    public static class glm
    {
        ...
        public static double IsPointBetweenAB(double start_x, double start_y, double end_x, double end_y,
          double point_x, double point_y)
        {
            double Sx = (point_x - start_x) / end_x;
            double Sy = (point_y - start_y) / end_y;
            double innerProduct = Sx - Sy;
            return innerProduct;
        }
        ...
        public static double HeadingToGrad(double Angle_Heading)
        {
            ...
            { Angle_Heading += glm.twoPI; }
            Angle_Degrees = glm.twoPI - Angle_Heading + glm.PIBy2;
            while (Angle_Degrees >= glm.twoPI)
            { Angle_Degrees -= glm.twoPI; }
            while (Angle_Degrees < 0)
            { Angle_Degrees += glm.twoPI; }
            return Angle_Degrees;
        }

        public static double Radiant0To2Pi(double Angle_Heading)
        {
            while (Angle_Heading >= glm.twoPI)
            { Angle_Heading -= glm.twoPI; }
            while (Angle_Heading < 0)
        ...
        public static double Distance(vecRoll first, vecRoll second)
        {
            return Math.Sqrt(
                Math.Pow(first.easting - second.easting, 2)
                + Math.Pow(first.northing - second.northing, 2));
