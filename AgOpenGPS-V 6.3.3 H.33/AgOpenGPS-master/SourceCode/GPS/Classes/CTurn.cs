    ...
    public partial class CBoundary
    {
        ...
        public void BuildTurnLines()
        {
            ...
            /*   if (bndList.Count == 0)
               ...
               double totalHeadWidthA = Properties.Settings.Default.set_youTurnDistanceFromBoundaryA;
               double totalHeadWidthB = Properties.Settings.Default.set_youTurnDistanceFromBoundaryB;
               ...
               for (int j = 0; j < bndList.Count; j++)
               {
                   ...
                   double ptcounthalf = ptCount / 2;

                  /* Console.WriteLine(" ptCount: " + ptCount);
                   Console.WriteLine(" ptcounthalf: " + ptcounthalf);
                   Console.WriteLine(" totalHeadWidth : " + totalHeadWidth);
                   Console.WriteLine(" totalHeadWidthA: " + totalHeadWidthA);
                   Console.WriteLine(" totalHeadWidthB: " + totalHeadWidthB);
  */ /*
            for (int i = ptCount - 1; i >= 0; i--)
            {
                ...
                */              /*                   if (i < ptcounthalf)
                                       {
                                           point.easting = bndList[j].fenceLine[i].easting + (-Math.Sin(glm.PIBy2 + bndList[j].fenceLine[i].heading) * totalHeadWidthA);
                                           point.northing = bndList[j].fenceLine[i].northing + (-Math.Cos(glm.PIBy2 + bndList[j].fenceLine[i].heading) * totalHeadWidthA);
                                           point.heading = bndList[j].fenceLine[i].heading;
                                           if (point.heading < -glm.twoPI) point.heading += glm.twoPI;
                                       }
                                       if (i >= ptcounthalf)
                                       {
                                           point.easting = bndList[j].fenceLine[i].easting + (-Math.Sin(glm.PIBy2 + bndList[j].fenceLine[i].heading) * totalHeadWidthB);
                                           point.northing = bndList[j].fenceLine[i].northing + (-Math.Cos(glm.PIBy2 + bndList[j].fenceLine[i].heading) * totalHeadWidthB);
                                           point.heading = bndList[j].fenceLine[i].heading;
                                           if (point.heading < -glm.twoPI) point.heading += glm.twoPI;
                                       }
                    */ /*
