...
namespace AgOpenGPS
{
    public class CTool
    {
        ...
        public double ToolantennaPivot = 1;
        public double ToolantennaOffset = 1;
        public double ToolantennaHeight = 1;
        public bool isToolAntenna3 = true;
        public vec3 pivotTool = new vec3();
        public vec3 AntennaToolOfset = new vec3();
        ...
        public void DrawToolAntenna()
        {
            if (mf.camera.camSetDistance > -200 && mf.isFirstHeadingSet)
            {
                //draw the bright antenna dot for antenna
                GL.PointSize(16);
                GL.Begin(PrimitiveType.Points);
                GL.Color3(1, 1, 0.98);
                GL.Vertex3(mf.tool.AntennaToolOfset.easting, -mf.tool.AntennaToolOfset.northing, 0.1);// white point of antenna
                GL.End();

                GL.PointSize(10);
                GL.Begin(PrimitiveType.Points);
                if (mf.tooltrk.isbtnAddToolTrackPts)
                    GL.Color3(0.1, 0.7, 0.1);   // green
                else GL.Color3(1.0, 0.2, 0.5);  // red
                GL.Vertex3(mf.tool.AntennaToolOfset.easting, -mf.tool.AntennaToolOfset.northing, 0.1);  // Tool point of antenna
                GL.End();

                GL.PointSize(16);
                GL.Begin(PrimitiveType.Points);
                GL.Color3(1, 1, 0.98);
                GL.Vertex3(mf.tool.pivotTool.easting, -mf.tool.pivotTool.northing, 0.1);// white pivot point of antenna
                GL.End();

                GL.PointSize(8);
                GL.Begin(PrimitiveType.Points);
                GL.Color3(0.90, 0.1, 0.98);
                GL.Vertex3(mf.tool.pivotTool.easting, -mf.tool.pivotTool.northing, 0.1);  // blue pivot point of antenna
                GL.End();
            }
        }
