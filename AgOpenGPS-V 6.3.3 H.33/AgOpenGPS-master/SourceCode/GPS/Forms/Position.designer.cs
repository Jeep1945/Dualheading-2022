    ...
    public partial class FormGPS
    {
        ...
        public void SC_MarkLinebyHand()
        {
            ...
            else if (trk.gArr[trk.idx].mode == (int)TrackMode.Curve) //4 = Curve
            {
                ...
                // violett drivingline
                for (int h = 0; h < curve.CurveroundPoints; h++)
                // curList[h].easting, curList[h].northing;
                {

                    //left side curveround
                    pos.easting = curve.curList[h].easting + (Math.Cos(curve.curList[h].heading) * +(tool.width / 2)) + (Math.Sin(curve.curList[h].heading));
                    pos.northing = curve.curList[h].northing + (Math.Sin(curve.curList[h].heading) * -(tool.width / 2)) + (Math.Cos(curve.curList[h].heading));
                    pos.heading = curve.curList[h].heading;
                    //add the point to List
                    triStripSC[selected_area].triangleList.Add(pos);
                    //Right side
                    pos.easting = curve.curList[h].easting + (Math.Cos(curve.curList[h].heading) * -(tool.width / 2)) + (Math.Sin(curve.curList[h].heading));
            ...
            else if (trk.gArr[trk.idx].mode == (int)TrackMode.bndCurve) //32 = bndCurve
            {
                int HowManyPathsAway = (int)curve.howManyPathsAway;
                if (HowManyPathsAway > 0) HowManyPathsAway++;
                ...
                int hj = curve.CurveroundPoints - 1;
                //for (int hj = curve.CurveroundPoints - 1; hj < curve.CurveroundPoints; hj++)
                {
                    //left side curveround
                    pos.easting = curve.curList[hj].easting + (Math.Cos(curve.curList[hj].heading) * +(tool.width / 2)) + (Math.Sin(curve.curList[hj].heading));
                    pos.northing = curve.curList[hj].northing + (Math.Sin(curve.curList[hj].heading) * -(tool.width / 2)) + (Math.Cos(curve.curList[hj].heading));
                    pos.heading = curve.curList[hj].heading;
                    //add the point to List
                    triStripSC[selected_area].triangleList.Add(pos);
                    //Right side
                    pos.easting = curve.curList[hj].easting + (Math.Cos(curve.curList[hj].heading) * -(tool.width / 2)) + (Math.Sin(curve.curList[hj].heading));
                ...
                // violett drivingline
                for (int h = 0; h < curve.CurveroundPoints; h += 2)
                {
                    //left side curveround
                    pos.easting = curve.curList[h].easting + (Math.Cos(curve.curList[h].heading) * +(tool.width / 2)) + (Math.Sin(curve.curList[h].heading));
                    pos.northing = curve.curList[h].northing + (Math.Sin(curve.curList[h].heading) * -(tool.width / 2)) + (Math.Cos(curve.curList[h].heading));
                ...
                int hi = curve.CurveroundPoints - 1;
                {
                    //left side curveround
                    pos.easting = curve.curList[hi].easting + (Math.Cos(curve.curList[hi].heading) * +(tool.width / 2)) + (Math.Sin(curve.curList[hi].heading));
                    pos.northing = curve.curList[hi].northing + (Math.Sin(curve.curList[hi].heading) * -(tool.width / 2)) + (Math.Cos(curve.curList[hi].heading));
                    pos.heading = curve.curList[hi].heading;
                    //add the point to List
                    triStripSC[selected_area].triangleList.Add(pos);
                    //Right side
                    pos.easting = curve.curList[hi].easting + (Math.Cos(curve.curList[hi].heading) * -(tool.width / 2)) + (Math.Sin(curve.curList[hi].heading));
