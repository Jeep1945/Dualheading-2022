...
namespace AgOpenGPS
{
    public partial class FormTramLine : Form
    {
        ...
        private void FormTramLine_Load(object sender, EventArgs e)
        {
            ...
            //Window Properties
            Size = (Size)Properties.Settings.Default.setWindow_tramLineSize;
        ...
        private void FormTramLine_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            Properties.Settings.Default.setWindow_tramLineSize = (Point)Size;
        ...
        private void LoadAndFixLines()
        {
            ...
            {
                if ((item.mode == (int)TrackMode.AB || item.mode == (int)TrackMode.Curve) && item.isVisible)
                {
                    //default side assuming built in AB Draw - isVisible is used for side to draw
                    gTemp.Add(new CTrk(item));
                    if (item.mode == (int)TrackMode.AB) 
        ...
        {
            if (gTemp[indx].mode == (int)TrackMode.Curve)
            {
                //if (Dist != 0)
                //mf.trk.NudgeRefCurve(Dist);
                BuildCurveTram();
            }
            else if (gTemp[indx].mode == (int)TrackMode.AB)
            {
                //if (Dist != 0)
