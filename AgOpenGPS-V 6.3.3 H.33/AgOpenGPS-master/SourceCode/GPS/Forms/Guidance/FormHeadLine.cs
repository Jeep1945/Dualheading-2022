...
namespace AgOpenGPS
{
    public partial class FormHeadLine : Form
    {
        ...
        private void btnDeletePoints_Click(object sender, EventArgs e)
        {
            ...
            mf.bnd.isHeadlandOn = !mf.bnd.isHeadlandOn;
            if (mf.vehicle.isHydLiftOn && !mf.bnd.isHeadlandOn) mf.vehicle.isHydLiftOn = false;

            if (!mf.bnd.isHeadlandOn)
            {
                mf.p_239.pgn[mf.p_239.hydLift] = 0;
                mf.btnHydLift.Image = Properties.Resources.HydraulicLiftOff;
            }
