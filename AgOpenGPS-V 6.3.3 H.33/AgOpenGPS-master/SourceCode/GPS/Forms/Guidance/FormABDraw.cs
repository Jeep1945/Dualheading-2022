    ...
    public partial class FormABDraw : Form
    {
        ...
        {            //count the points from the boundary
            ...
            if (cnt > 3)
            {
                ...
                //create a name
                if (bndSelect > 0)
                    gTemp[indx].name = "inner Bnd " + bndSelect.ToString();
                else
                    gTemp[indx].name = "outer Bnd " + bndSelect.ToString();

                //if (q > 0) gTemp[indx].name = "Boundary Curve " + q.ToString();
        ...
        private void btnSkipBackBnd_Click(object sender, EventArgs e)
        {
            //mf.bnd.bndList[bndSelect]
            if (bndSelect < 0)
            {
                return;
            }
            ...
            {
                bndSelect--;
                if (bndSelect < 0) bndSelect = mf.bnd.bndList.Count - 1;
            }
            if (bndSelect > 0)
                tboxNameBnd.Text = "inner Bnd " + bndSelect.ToString();
            else
                tboxNameBnd.Text = "outer Bnd " + bndSelect.ToString();

            btnMakeBoundaryCurve.Enabled = true;

        }

        private void btnSkipForwardBnd_Click(object sender, EventArgs e)
        {
            if (bndSelect < 0)
            {
                return;
            ...
            {
                bndSelect++;
                if (bndSelect > mf.bnd.bndList.Count - 1) bndSelect = 0;
            }
            if (bndSelect > 0)
                tboxNameBnd.Text = "inner Bnd " + bndSelect.ToString();
            else
